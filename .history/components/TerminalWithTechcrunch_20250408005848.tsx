...
class Ring {
  ...
  constructor(origin: Vector3, radius: number, rotation: Vector3, count: number, container: HTMLDivElement) {
    ...
    this.rotationSpeed = 0.005 + Math.random() * 0.005; // Slightly different speeds for each ring
  ...
  updateRotation() {
    // Increment rotation angle (clockwise rotation)
    ...
    this.particles.forEach(p => {
      ...
      // Update the element's position if already rendered
      if (p.el.style.left.includes('px')) {
