...
type ParticleData = {
  ...
  hasGlow: boolean;
  angle: number; // Added to track particle's angle
};
...
class Ring {
  ...
  rotationSpeed: number; // Added for rotation speed
  rotationAngle: number = 0; // Added to track current rotation
  ...
  constructor(origin: Vector3, radius: number, rotation: Vector3, count: number, container: HTMLDivElement) {
    ...
    this.rotationSpeed = 0.005 + Math.random() * 0.005; // Random speed between 0.005 and 0.01
  }
  ...
  updateRotation() {
    this.rotationAngle += this.rotationSpeed; // Increment rotation angle
...
export default function TerminalWithTechcrunch() {
  ...
  useEffect(() => {
    ...
    // Animation loop for rotating rings
    const animate = () => {
