...
export default function TerminalWithTechcrunch() {
  ...
  useEffect(() => {
    ...
    const createRing = (ringRef: typeof ring1Particles, radius: number, zIndex: number) => {
      for (let i = 0; i < OUTSIDE_PARTICLE_COUNT; i++) {
        ...
        // 3D positions (perspective effect)
        const angle = (i * 2 * Math.PI) / OUTSIDE_PARTICLE_COUNT;
        const tiltAngle = Math.PI / 6; // 30 degree tilt
        const x = centerX + radius * Math.cos(angle);
        const y = centerY + radius * Math.sin(angle) * Math.cos(tiltAngle);
        const z = radius * Math.sin(tiltAngle) * Math.sin(angle);
    ...
    // Create three rings with different radii and z-indices
    createRing(ring1Particles, RING_RADIUS * 0.8, 5);  // Front ring
    createRing(ring2Particles, RING_RADIUS * 1.0, 0);  // Middle ring (terminal level)
    createRing(ring3Particles, RING_RADIUS * 1.2, -5); // Back ring
  ...
  // Turbo mode transition with 3D effect
  useEffect(() => {
    const updateRingParticles = (ringRef: typeof ring1Particles) => {
      ringRef.current.forEach((p, i) => {
        ...
        if (isTurboMode) {
          ...
          const scale = 1 + p.turbo.z / (RING_RADIUS * 2);
          span.style.transform = `translate3d(${p.turbo.x}px, ${p.turbo.y}px, ${p.turbo.z}px) scale(${scale})`;
          span.style.opacity = `${0.7 + (p.turbo.z / (RING_RADIUS * 2)) * 0.3}`;
          span.style.filter = `blur(${Math.abs(p.turbo.z) / 50}px)`;
        } else {
    ...
    if (containerRef.current) {
      containerRef.current.style.perspective = isTurboMode ? "1000px" : "none";
      containerRef.current.style.transformStyle = isTurboMode ? "preserve-3d" : "flat";
    }
  ...
  return (
    ...
    >
      {/* Backdrop for depth */}
      {isTurboMode && (
        <div className="absolute inset-0 bg-gradient-to-b from-purple-900/20 to-black/50 z-[-1]" />
      )}
      ...
      <div
        ...
        style={{
          ...
          overflow: "hidden",
          transform: isTurboMode ? "translateZ(0)" : "none",
      ...
      >
        ...
        <h1
          ...
          style={{
            ...
            transform: isTurboMode ? "translateZ(20px)" : "none",
            transition: "transform 0.5s ease-out"
          }}
        >
          TECHCRUNCH
        ...
        <div 
          className="absolute -top-12 -left-12 w-72 h-72 bg-purple-200 rounded-full opacity-30 blur-3xl animate-pulse pointer-events-none z-0"
          style={{
        ...
        />
        <div 
          className="absolute bottom-[-3rem] right-[-2rem] w-48 h-48 bg-pink-200 rounded-full opacity-20 blur-2xl animate-spin-slow pointer-events-none z-0"
          style={{
            transform: isTurboMode ? "translateZ(100px)" : "none",
            transition: "transform 0.5s ease-out"
          }}
