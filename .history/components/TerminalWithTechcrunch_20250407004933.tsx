    ...
    outsideParticlesRef.current.forEach((particle, i) => {
      if (isTurboMode) {
        if (i < groupSize) {
          ...
          // Second group: X-axis oriented ellipse
          const progress = (i - groupSize) / groupSize;
          const x = centerX + X_AXIS_RADIUS * Math.cos(progress * Math.PI * 2);
          const y = centerY + Y_AXIS_RADIUS * Math.sin(progress * Math.PI * 2);
          particle.style.left = `${x}px`;
          particle.style.top = `${y}px`;
        } else {
          // Third group: Cross shape (X)
          const angle = ((i - groupSize * 2) * 2 * Math.PI) / (OUTSIDE_PARTICLE_COUNT - groupSize * 2);
          const radius = RING_RADIUS * 0.7;
          const x = centerX + radius * Math.cos(angle) * Math.abs(Math.cos(angle));
          const y = centerY + radius * Math.sin(angle) * Math.abs(Math.sin(angle));
          particle.style.left = `${x}px`;
          particle.style.top = `${y}px`;
        }
