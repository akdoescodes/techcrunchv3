...
type ParticleData = {
  ...
  angle: number; // Store the initial angle for static positioning
  radius: number; // Store the radius for static positioning
};
  ...
  generateParticles(charList: string[]) {
    for (let i = 0; i < this.count; i++) {
      ...
      this.particles.push({
        ...
        angle, // Store the initial angle for static positioning
        radius: this.radius // Store the radius for static positioning
      });
    }
  }
...
export default function TerminalWithTechcrunch() {
  ...
  const ringConfigs = [
    ...
    { radius: 550, rotation: { x: 84, y: 180, z: 0 }, count: 60 },
  ...
  useEffect(() => {
    ringsRef.current.forEach(ring => {
      // Update positions based on turbo mode
      if (isTurboMode) {
      ...
      // Update visuals based on turbo mode
