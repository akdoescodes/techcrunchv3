  ...
  const positions = useMemo(() => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      pos.push({
        ...
        z: r * Math.cos(phi),
        scale: 0.02 + Math.random() * 0.03, // Smaller particles
...
function FloatingParticles({ count = 100, radius = 8 }) { // Increased count for more density
  const particlesRef = useRef<THREE.InstancedMesh>(null);
  const dummy = useMemo(() => new THREE.Object3D(), []);
  const particles = useMemo(() => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      temp.push({
        ...
        scale: 0.01 + Math.random() * 0.02, // Smaller particles
  ...
  return (
    <instancedMesh ref={particlesRef} args={[null, null, count]}>
      <sphereGeometry args={[1, 6, 6]} /> {/* Lower resolution for performance */}
      <meshStandardMaterial
        color="#00FF00" // Green color
        emissive="#00AA00"
  ...
  return (
    <div className="w-full h-screen" style={{
      ...
      <Canvas shadows camera={{ position: [0, 0, 12], fov: 45 }}>
        ...
        <pointLight position={[10, 10, 10]} intensity={1.2} color="#8000FF" />
        ...
        {/* Background particles - now green and more numerous */}
        <FloatingParticles count={150} radius={10} />
