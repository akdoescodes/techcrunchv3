...
const TECHCRUNCH_TEXT_SIZE = "3rem"; // Adjust title size here
...
export default function TerminalWithTechcrunch() {
  ...
  useEffect(() => {
    ...
    // Set terminal as reference frame
    const { offsetWidth, offsetHeight } = terminal;
    ...
    for (let i = 0; i < PARTICLE_COUNT; i++) {
      ...
      span.style.left = `${Math.random() * offsetWidth}px`;
      span.style.top = `${Math.random() * offsetHeight}px`;
  ...
  return (
    <div className="relative w-full px-4 py-10 flex items-center justify-center overflow-visible">
      <div
        ref={terminalRef}
        className="relative flex flex-col justify-start rounded-lg border border-purple-700 shadow-[0_0_2px_#a855f766] backdrop-blur-sm text-gray-200 font-mono text-sm px-6 py-6 z-10 overflow-hidden"
        style={{
          width: customWidth || "100%",
          maxWidth: "720px",
          minHeight: customHeight || "24rem",
