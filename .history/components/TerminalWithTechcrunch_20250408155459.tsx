...
class Ring {
  ...
  generateParticles(charList: string[]) {
    for (let i = 0; i < this.count; i++) {
      ...
      const size = 1.5 + Math.random() * 2; // Increased size range
      ...
      if (hasGlow) {
        span.style.color = `hsl(${hue}, ${saturation}%, 90%)`; // Brighter color
        span.style.textShadow = `0 0 ${15 + Math.random() * 10}px hsl(${hue}, ${saturation}%, 90%)`; // Increased glow
      } else {
  ...
  animateToPositions() {
    ...
    const animate = (timestamp: number) => {
      ...
      this.particles.forEach((p) => {
        ...
        p.el.style.opacity = `${easedProgress * 0.9}`; // Increased final opacity
      });
...
export default function TerminalWithTechcrunch() {
  ...
  const ringConfigs = [
    { radius: 460, count: 40, speed: 1.2, spinSpeed: 0.05 },
    { radius: 300, count: 30, speed: 1.0, spinSpeed: -0.08 },
    { radius: 550, count: 25, speed: 0.8, spinSpeed: 0.04 },
    { radius: 380, count: 35, speed: 1.1, spinSpeed: -0.06 },
  ];
