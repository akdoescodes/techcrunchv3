...
function OrbitRing({ tilt = 0, radius = 4, color = "#00FFFF" }) {
...
function GradientText() {
  ...
  useMemo(() => {
    if (materialRef.current) {
      materialRef.current.onBeforeCompile = (shader: any) => {
        shader.fragmentShader = shader.fragmentShader.replace(
          `vec4 diffuseColor = vec4( diffuse, opacity );`,
          `
            vec3 gradient = mix(vec3(1.0, 0.0, 1.0), vec3(0.0, 1.0, 1.0), (position.y + 5.0) / 10.0);
