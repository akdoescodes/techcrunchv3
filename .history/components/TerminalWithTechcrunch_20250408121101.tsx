...
class StaticRing {
  ...
  generateParticles(charList: string[]) {
    for (let i = 0; i < this.count; i++) {
      ...
      // For non-turbo mode, we'll position particles randomly
...
export default function TerminalWithTechcrunch() {
  ...
  const ringsRef = useRef<StaticRing[]>([]);

  // Create more complex ring configurations
  const ringConfigs = [
    // Main rings
    { radius: 450, rotation: { x: 84, y: 0, z: 30 }, count: 80 },
    { radius: 300, rotation: { x: 30, y: 80, z: 10 }, count: 65 },
    { radius: 550, rotation: { x: 90, y: 180, z: 0 }, count: 70 },
    { radius: 400, rotation: { x: 75, y: 0, z: -30 }, count: 60 },
    
    // Additional rings for complex geometry
    { radius: 200, rotation: { x: 45, y: 45, z: 45 }, count: 40 },
    { radius: 350, rotation: { x: 10, y: 100, z: 60 }, count: 50 },
    { radius: 480, rotation: { x: 60, y: 30, z: 90 }, count: 75 },
    { radius: 250, rotation: { x: 120, y: 0, z: 20 }, count: 35 },
    
    // Intersecting rings
    { radius: 420, rotation: { x: 0, y: 90, z: 45 }, count: 55 },
    { radius: 320, rotation: { x: 150, y: 30, z: 0 }, count: 45 },
    { radius: 500, rotation: { x: 20, y: 70, z: 120 }, count: 65 },
    { radius: 380, rotation: { x: 100, y: 100, z: 100 }, count: 50 }
  ];
  ...
  useEffect(() => {
    ...
    ringConfigs.forEach(cfg => {
      const ring = new StaticRing(center, cfg.radius, cfg.rotation, cfg.count, container);
  ...
  useEffect(() => {
    ringsRef.current.forEach(ring => {
      ring.particles.forEach(p => {
        ...
        // Toggle between original and turbo positions
        if (isTurboMode) {
