...
const wireConfigs = positions.flatMap((end, i) => {
  const offsets = [-screenWidth / 2.5, screenWidth / 2.5];
  return offsets.map((xOffset, j) => {
    const endVec = new THREE.Vector3(0, end[1], 0);
    const startVec = new THREE.Vector3(xOffset, end[1], 0);
    const curveStrength = 300 + Math.abs(2 - i) * 120;
    return {
      points: generateCurveOutward(startVec, endVec, curveStrength),
      width: 0.035 + ((i + j) % 3) * 0.005,
    };
  });
});
