...
export default function TerminalWithTechcrunch() {
 ...
 // Turbo mode 3D transition
useEffect(() => {
  const centerX = window.innerWidth / 2;
  const centerY = window.innerHeight / 2;
  const DEPTH_RANGE = 300; // how "deep" the 3D effect is
  const RING_RADIUS = 220; // orbit radius
  const MIN_VISIBLE_Z = 0; // hide behind-the-terminal particles
  ...
  const updateRingParticles = (ringRef: typeof ring1Particles, offsetAngle = 0) => {
    ringRef.current.forEach((p, i) => {
      ...
      if (isTurboMode) {
        ...
        const ringZ = DEPTH_RANGE * Math.sin(angle * 2); // back and forth Z

        // 3D positioning
        span.style.transform = `translate3d(${centerX + ringX}px, ${centerY + ringY}px, ${ringZ}px)`;
        span.style.opacity = ringZ >= MIN_VISIBLE_Z ? "0.9" : "0"; // hide behind particles
      } else {
        // Go back to original 2D ring layout
        span.style.left = `${p.original.x}px`;
        span.style.top = `${p.original.y}px`;
        span.style.transform = `translate3d(0, 0, 0)`; // reset transform
  ...
  updateRingParticles(ring2Particles, Math.PI / 3); // rotate for variation
