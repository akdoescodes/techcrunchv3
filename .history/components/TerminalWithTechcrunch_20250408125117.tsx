...
// Center offset configuration to make it adjustable
type CenterConfig = {
  offsetX: number; // Percentage offset from center (-50 to 50)
  offsetY: number; // Percentage offset from center (-50 to 50)
  useFixed: boolean; // Use fixed values instead of percentages
  fixedX: number; // Fixed X position in pixels
  fixedY: number; // Fixed Y position in pixels
};
...
class Ring {
  ...
  constructor(origin: Vector3, radius: number, rotation: Vector3, count: number, container: HTMLDivElement, centerConfig: CenterConfig) {
    this.origin = origin;
    this.radius = radius;
...
export default function TerminalWithTechcrunch() {
  ...
  const [centerConfig, setCenterConfig] = useState<CenterConfig>({
    offsetX: 0,     // Percentage offset from center horizontally (-50 to 50)
    offsetY: 0,     // Percentage offset from center vertically (-50 to 50)
    useFixed: false, // Set to true to use fixed values instead of percentages
    fixedX: 350,    // Fixed X position in pixels
    fixedY: 250     // Fixed Y position in pixels
  });
  ...
  // Function to calculate and update center position
  const updateCenterPosition = () => {
    ...
    if (centerConfig.useFixed) {
      centerX = centerConfig.fixedX;
      centerY = centerConfig.fixedY;
    } else {
      // Calculate based on percentage offsets
      centerX = terminalWidth / 2 + (terminalWidth * centerConfig.offsetX / 100);
      centerY = terminalHeight / 2 + (terminalHeight * centerConfig.offsetY / 100);
    }
  ...
  const updateRingsCenterPositions = () => {
    const center = updateCenterPosition();
    if (center) {
      ringsRef.current.forEach(ring => {
  ...
  // Control panel component for adjusting center position
  const CenterControls = () => (
    <div className="absolute left-4 bottom-4 text-xs z-20 flex flex-col">
      <div className="mb-2 flex items-center">
        <input
          type="checkbox"
          checked={centerConfig.useFixed}
          onChange={(e) => {
            setCenterConfig({
              ...centerConfig,
              useFixed: e.target.checked
            });
          }}
          className="mr-2"
        />
        <label className="text-gray-300">Use Fixed Position</label>
      </div>
      
      {centerConfig.useFixed ? (
        <>
          <div className="flex items-center mb-1">
            <span className="w-8 text-gray-400">X:</span>
            <input
              type="number"
              value={centerConfig.fixedX}
              onChange={(e) => {
                setCenterConfig({
                  ...centerConfig,
                  fixedX: Number(e.target.value)
                });
              }}
              className="w-16 bg-purple-900 border border-purple-500 text-white text-xs py-1 px-1 rounded"
            />
          </div>
          <div className="flex items-center">
            ...
            <input
              type="number"
              value={centerConfig.fixedY}
              onChange={(e) => {
                setCenterConfig({
                  ...centerConfig,
                  fixedY: Number(e.target.value)
                });
              }}
              className="w-16 bg-purple-900 border border-purple-500 text-white text-xs py-1 px-1 rounded"
            />
        ...
        <>
          <div className="flex items-center mb-1">
            ...
            <input
              ...
              className="w-24"
            />
            <span className="ml-1 text-gray-400">{centerConfig.offsetX}%</span>
          </div>
          <div className="flex items-center">
            <span className="w-8 text-gray-400">Y%:</span>
            <input
              ...
              className="w-24"
            />
            <span className="ml-1 text-gray-400">{centerConfig.offsetY}%</span>
          </div>
        </>
      )}
    </div>
  ...
  // Effect to update center position when config changes
  useEffect(() => {
    updateRingsCenterPositions();
  }, [centerConfig, isTurboMode]);
  ...
  useEffect(() => {
    ...
    ringConfigs.forEach(cfg => {
      const ring = new Ring(center, cfg.radius, cfg.rotation, cfg.count, container, centerConfig);
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Control panel for adjusting center position */}
        <CenterControls />
