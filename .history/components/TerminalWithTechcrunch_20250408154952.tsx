...
const INSIDE_PARTICLE_COUNT = 30; // Reduced from 50
...
class Ring {
  ...
  generateParticles(charList: string[]) {
    for (let i = 0; i < this.count; i++) {
      ...
      const size = 1 + Math.random() * 2; // Increased size range
      span.style.fontSize = `${size * 0.75}rem`;

      // Glow effects with more variation
      const hasGlow = Math.random() < 0.5; // Increased probability for glow
      const hue = 60 + Math.random() * 40;
      const saturation = 80 + Math.random() * 20;

      if (hasGlow) {
        span.style.color = `hsl(${hue}, ${saturation}%, 80%)`; // Brighter color
        span.style.textShadow = `0 0 ${10 + Math.random() * 10}px hsl(${hue}, ${saturation}%, 80%)`; // Increased glow
      } else {
        span.style.color = `hsl(${hue}, ${saturation}%, ${60 + Math.random() * 20}%)`; // Brighter base color
      }
      ...
      this.particles.push({
        ...
        delay: Math.random() * 0.3, // Reduced delay for quicker appearance
        currentOrbitAngle: Math.random() * Math.PI * 2,
        orbitRadius: 10 + Math.random() * 20, // Increased orbit radius
  ...
  animateToPositions() {
    ...
    const animate = (timestamp: number) => {
      ...
      this.particles.forEach((p) => {
        ...
        const progress = Math.max(0, Math.min(1, (elapsed / 1000 - p.delay) / 1.5));
        ...
        // Update orbit angle
        p.currentOrbitAngle += 0.002 * p.spinDirection;
        ...
        p.el.style.transform = `translate(${orbitX}px, ${orbitY}px) scale(${0.8 + easedProgress * 0.4})`;
      ...
      this.globalAngle += this.globalSpinSpeed * 0.01;

      if (elapsed < 1000) { // Shorter animation duration for quicker appearance
...
export default function TerminalWithTechcrunch() {
  ...
  const ringConfigs = [
    { radius: 460, count: 40, speed: 1.2, spinSpeed: 0.05 }, // Reduced count
    { radius: 300, count: 30, speed: 1.0, spinSpeed: -0.08 }, // Reduced count
    { radius: 550, count: 25, speed: 0.8, spinSpeed: 0.04 }, // Reduced count
    { radius: 380, count: 35, speed: 1.1, spinSpeed: -0.06 }, // Reduced count
  ];
  ...
  useEffect(() => {
    ...
    // Create inside particles with reduced count
    for (let i = 0; i < INSIDE_PARTICLE_COUNT; i++) {
      ...
      const hasGlow = Math.random() < 0.2; // Reduced probability
