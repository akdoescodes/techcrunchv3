...
class ParticleRing {
  ...
  initialize(container, avoidRect, characters) {
    ...
    // Pre-calculate all base ring positions
    this.basePositions = Array.from({ length: this.particleCount }, (_, i) => {
    ...
    for (let i = 0; i < this.particleCount; i++) {
      ...
      this.particles.push({
        element: span,
        originalPos: { x, y },
        basePos: this.basePositions[i]
      });
    }
  }

  applyRotation(basePos) {
    const { x, y, z } = basePos;
    const { x: rotX, y: rotY } = this.rotation;
    ...
    // X-axis rotation
    const xRotY = y * Math.cos(rotX) - yRotZ * Math.sin(rotX);
    const xRotZ = yRotZ * Math.cos(rotX) + y * Math.sin(rotX);
  ...
  updatePositions() {
    ...
    this.particles.forEach((particle, i) => {
      const { x, y, z } = this.applyRotation(particle.basePos);
      const scale = 1 + z / 1000;
      const opacity = 0.5 + 0.5 * (1 - Math.abs(z) / this.radius);
