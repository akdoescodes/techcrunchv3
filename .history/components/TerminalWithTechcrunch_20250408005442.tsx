...
class Ring {
  ...
  generateParticles(charList: string[]) {
    for (let i = 0; i < this.count; i++) {
      ...
      span.style.position = "absolute"; // Ensure positioning works
      ...
      span.style.opacity = "0.5"; // Initial opacity
  ...
  updateRotation() {
    this.currentAngle += this.rotationSpeed * (Math.PI / 180);
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < INSIDE_PARTICLE_COUNT; i++) {
      ...
      if (hasGlow) {
        ...
        span.style.animation = `glowPulseInside ${4 + Math.random() * 5}s infinite alternate, floatSoft ${6 + Math.random() * 6}s infinite`;
