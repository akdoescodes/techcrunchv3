...
type ParticleData = {
  ...
  hasGlow: boolean;
  angle: number; // Track particle's initial angle
};
...
class Ring {
  ...
  rotationAngle: number = 0; // Current rotation angle
  ...
  constructor(origin: Vector3, radius: number, rotation: Vector3, count: number, container: HTMLDivElement) {
    ...
    // Increase rotation speed to make it more noticeable
  ...
  generateParticles(charList: string[]) {
    for (let i = 0; i < this.count; i++) {
      ...
      this.particles.push({
        ...
        hasGlow,
        angle // Store initial angle for rotation
      });
    }
  }
  ...
  updateRotation() {
    ...
    this.particles.forEach((p, index) => {
      // Calculate new position based on initial angle plus the current rotation
      const newAngle = p.angle + this.rotationAngle;
      ...
      // Update the turbo position
      p.turbo = { x: finalX, y: finalY };
      
      // Directly update the element's position (no transition for rotation)
...
export default function TerminalWithTechcrunch() {
  ...
  useEffect(() => {
    ...
    // Animation loop - runs continuously regardless of mode
    const animate = () => {
      // Only update rotation if in turbo mode
      if (isTurboMode) {
  ...
  useEffect(() => {
    // This effect handles the transition between modes
    ringsRef.current.forEach(ring => {
      ring.particles.forEach(p => {
        const el = p.el;
        
        if (!isTurboMode) {
          // When turning OFF turbo mode, return to original positions
          ...
          // When turning ON turbo mode, switch to the organized positions
          el.style.left = `${p.turbo.x}px`;
          el.style.top = `${p.turbo.y}px`;
          el.style.opacity = "0.8";
          el.style.transform = "rotateX(45deg)";
          // Remove transition when in turbo mode so rotations are smooth
