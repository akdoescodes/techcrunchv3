...
function TechText() {
    ...
    const geometry = new TextGeometry("TECHCRUNCH", config);
    geometry.computeBoundingBox();
  
    const centerOffsetX =
      geometry.boundingBox ? -0.5 * (geometry.boundingBox.max.x - geometry.boundingBox.min.x) : 0;
    const centerOffsetY =
      geometry.boundingBox ? -0.5 * (geometry.boundingBox.max.y - geometry.boundingBox.min.y) : 0;
  
    return (
      <mesh ref={meshRef} geometry={geometry} position={[centerOffsetX, centerOffsetY, 0]}>
