...
function generateButterflyCurve(start: THREE.Vector3, end: THREE.Vector3, curveStrength = 1, widthScale = 1): THREE.Vector3[] {
  ...
  for (let i = 0; i <= segments; i++) {
    ...
    const x = Math.sin(angle) * (Math.exp(Math.cos(angle)) - 2 * Math.cos(4 * angle) - Math.pow(Math.sin(angle / 12), 5)),
    const y = Math.cos(angle) * (Math.exp(Math.cos(angle)) - 2 * Math.cos(4 * angle) - Math.pow(Math.sin(angle / 12), 5),
    const z = 0;
...
const wireConfigs = Array.from({ length: 12 }, (_, i) => {
  const angle = (i / 12) * Math.PI * 2;
  const radius = 1.5 + 0.2 * i;
  const x = Math.cos(angle) * radius;
  const y = Math.sin(angle) * radius;
  const start = new THREE.Vector3(x, y, 0);
  const end = new THREE.Vector3(-x, -y, 0);
  return {
    points: generateButterflyCurve(start, end, 3 + i * 0.3, 1.5),
