...
class HungryInsideParticle {
  ...
  moveToward(target: HTMLElement) {
    const rect = target.getBoundingClientRect();
    const spanRect = this.el.getBoundingClientRect();
    const dx = rect.left + rect.width / 2 - (spanRect.left + spanRect.width / 2);
    const dy = rect.top + rect.height / 2 - (spanRect.top + spanRect.height / 2);
...
class EatEffectController {
  particles: HungryInsideParticle[] = [];
  targets: HTMLElement[] = [];
  intervalId: number | null = null;

  constructor(particles: HTMLSpanElement[], targets: HTMLElement[]) {
    this.particles = particles.map(p => new HungryInsideParticle(p));
...
export default function TerminalWithTechcrunch() {
  ...
  const controllerRef = useRef<EatEffectController | null>(null);
  ...
  useEffect(() => {
    ...
    if (isTurboMode) {
      controllerRef.current = new EatEffectController(insideParticlesRef.current, letterRefs.current);
